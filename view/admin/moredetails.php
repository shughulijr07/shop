...
$today = date("Y-m-d");
$today_sells = $dBcONNECT->connect()->prepare("SELECT SUM(totalSelling) as total_sell FROM issue WHERE date_issued = '$today' AND shop = s$shop_pointhop_point ");
$today_sells->execute();
...
$shop_point_sells = $dBcONNECT->connect()->prepare("SELECT * FROM issue WHERE date_issued = '$today' AND shop = s$shop_pointhop_point");
