...
{	
	...
	function header(){
      ...
      date_default_timezone_set("Africa/Dar_es_Salaam");
        ...
        $today_sells = $database->connect()->prepare("SELECT SUM(totalSelling) as total_sell FROM issue WHERE date_issued = '$today' AND shop = '$shop'");
        $today_sells->execute();
	...
	function headerTable(){
		...
		$this->Cell(85,10,'productduct',1,0,'C');
	...
	function SlipDetails()
	{
        ...
        $database = new DatabaseConnection;
        $shop = $_REQUEST['shop'];
        $query = $database->connect()->query("SELECT * FROM issue WHERE shop = '$shop' AND date_issued = '$today'");
        while($data = $query->fetch(PDO::FETCH_OBJ))
		{
		$this->SetFont('Times','',12);
		$this->Cell(85,10,$data->product,1,0,'L');
		$this->Cell(60,10,$data->type,1,0,'L');
        $this->Cell(40,10,number_format($data->price),1,0,'L');
        $this->Cell(30,10,$data->qty,1,0,'L');
        $this->Cell(60,10,number_format($data->totalSelling),1,0,'L');
